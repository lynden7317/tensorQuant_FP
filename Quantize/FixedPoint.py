...
round_module_floating = tf.load_op_library('../Kernels/RoundOp_float.so')
...
def round_floating(input1, exp_bit, mant_bit):
    ''' Truncates input1 to a fixed point number.
        Rounds randomly, but the fractional part influences the probability.'''
    result = round_module_floating.round_float(input1,exp_bit=exp_bit,mant_bit=mant_bit)
    return result
@ops.RegisterGradient("RoundFloat")
def _round_floating_grad(op, grad):
  return [grad]
